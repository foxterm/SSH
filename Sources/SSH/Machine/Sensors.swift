...
public extension Machine {
    func getTemp() async -> [TemperatureStat]? {
        let gatherCmd = #"found=0; if [ -d /sys/class/hwmon ]; then for d in /sys/class/hwmon/hwmon*; do [ -d "$d" ] || continue; name=$(cat "$d/name" 2>/dev/null || echo "unknown"); for input in "$d"/temp*_input; do [ -f "$input" ] || continue; val=$(cat "$input" 2>/dev/null); if [ -n "$val" ] && [ "$val" -ne 0 ]; then base=${input%_input}; echo "hw|$name|$val|$(cat "${base}_crit" 2>/dev/null || echo 0)|$(cat "${base}_max" 2>/dev/null || echo 0)|$(cat "${base}_label" 2>/dev/null || echo "")"; found=1; fi; done; done; fi; if [ "$found" -eq 0 ] && [ -d /sys/class/thermal ]; then for d in /sys/class/thermal/thermal_zone*; do [ -d "$d" ] || continue; type=$(cat "$d/type" 2>/dev/null || echo "unknown"); temp=$(cat "$d/temp" 2>/dev/null || echo 0); if [ "$temp" -ne 0 ]; then echo "tz|$type|$temp|0|0|none"; fi; done; fi"#
