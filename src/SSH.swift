...
public class SSH {
    ...
    public static let banner =
        "SSH-2.0-libssh2_\(LIBSSH2_VERSION_MAJOR).\(LIBSSH2_VERSION_MINOR).\(LIBSSH2_VERSION_PATCH)-FoxTerm_\(Bundle.main.infoDictionary?["CFBundleShortVersionString"] ?? "")"
    ...
    let tracCallback: libssh2_trace_handler_func = { session, _, message, messageLen in
        ...
        libssh2_session_abstract(session).address.ssh.trace(message: message, messageLen: messageLen)
    }

    /// 远程断开连接通知回调
    let disconnectCallback: disconnectType = { sess, reason, message, messageLen, language, languageLen, abstract in
