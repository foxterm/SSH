// FoxTerm | Sensors.swift
// Copyright (c) 2025-2026 foxterm.app
// Created by foxterm@foxmail.com

import Foundation

public extension Machine {
    /// 获取远程主机的硬件温度统计信息
    /// 兼容多种 Linux 内核温度读取接口：hwmon (主流) 与 thermal_zone (备用)
    /// - Returns: 温度统计数据数组，若无法读取则返回 nil
    func getTemp() async -> [TemperatureStat]? {
        let gatherCmd = """
        if [ -d /sys/class/hwmon ] && [ "$(ls -A /sys/class/hwmon 2>/dev/null)" ]; then
            for d in /sys/class/hwmon/hwmon*; do
                [ -d "$d" ] || continue
                name=$(cat "$d/name" 2>/dev/null || echo "unknown")
                for input in "$d"/temp*_input; do
                    [ -f "$input" ] || continue
                    base="${input%_input}"
                    echo "hw|${name}|$(cat "$input" 2>/dev/null)|$(cat "${base}_crit" 2>/dev/null || echo 0)|$(cat "${base}_max" 2>/dev/null || echo 0)|$(cat "${base}_label" 2>/dev/null || echo "")"
                done
            done
        elif [ -d /sys/class/thermal ]; then
            for d in /sys/class/thermal/thermal_zone*; do
                [ -d "$d" ] || continue
                echo "tz|$(cat "$d/type" 2>/dev/null)|$(cat "$d/temp" 2>/dev/null)|0|0|none"
            done
        fi
        """

        // CPU 识别关键词列表
        let cpuKeywords = [
            "coretemp", "pkgtemp", "k10temp", "zenpower", "fam15h_power",
            "amd_energy", "cpu_thermal", "soc_thermal", "scpi_sensors",
            "cpu", "acpitz", "virt_temp", "package", "soc",
        ]

        guard let lines = await channel.exec(gatherCmd)?.string?.lines else { return nil }

        var ret: [TemperatureStat] = []

        for line in lines {
            let parts = line.components(separatedBy: "|")
            // 现在包含 6 个部分: source|name|temp|crit|max|label
            guard parts.count >= 6 else { continue }

            let name = parts[1].lowercased()
            let tempRaw = Double(parts[2]) ?? 0
            let critRaw = Double(parts[3]) ?? 0
            let maxRaw = Double(parts[4]) ?? 0
            let label = parts[5].lowercased()

            if tempRaw == 0 { continue }

            var t = TemperatureStat()
            t.name = name
            t.label = label
            t.temperature = tempRaw / 1000
            t.sensorCritical = critRaw > 0 ? critRaw / 1000 : 0
            t.sensorHigh = maxRaw > 0 ? maxRaw / 1000 : 0

            // 识别逻辑：驱动名命中 或 标签包含核心关键字
            let nameMatch = cpuKeywords.contains { name.contains($0) }
            let labelMatch = label.contains("core") || label.contains("pkg") || label.contains("package")

            t.cpu = nameMatch || labelMatch
            ret.append(t)
        }

        return ret.isEmpty ? nil : ret
    }
}
