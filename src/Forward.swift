...
public extension Forward {
    ...
    /// - Returns: 转发通道是否成功建立
    func tcpip(host: String, port: Int, shost: String, sport: Int, read: InputStream, write: OutputStream) async -> Bool {
    ...
    ///   - sport: 源主机端口
    /// - Returns: 转发通道是否成功建立
    func streamLocal(socketPath: String, shost: String, sport: Int, read: InputStream, write: OutputStream) async -> Bool {
        guard rawSession != nil else {
            return false
        }
    ...
    internal func copy(read: InputStream, write: OutputStream) async {
        guard rawChannel != nil else {
            return
        }
        // 并发处理双向流：
