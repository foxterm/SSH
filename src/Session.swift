...
public extension SSH {
    ...
    /// 计算并返回服务器公钥的指纹字符串
    /// - Parameter type: 哈希类型，默认为 SHA1
    func fingerprint(_ type: HostkeyHash = .sha1) -> String? {
        guard rawSession != nil else { return nil }
        guard let hashPointer = libssh2_hostkey_hash(rawSession, type.value) else {
            return nil
        }
        return Data(bytes: hashPointer, count: type.length).fingerprint
    }
